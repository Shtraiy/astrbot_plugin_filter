...
class LanguageLogicOptimizer(Star):
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req) -> None:
        ...
        self._guard_text_only_attribution(event, req)
    ...
    def _guard_text_only_attribution(self, event: AstrMessageEvent, req) -> None:
        """Keep the model from attributing the bot's own media to the user.

        For every text-only user message, append a note clarifying that the
        user sent no image this round and that images in history belong to
        their sender role. This counters memory/history text that claims the
        user sent memes the bot actually sent itself.
        """
        ...
        try:
            ...
            if not user_has_media and append_text_only_media_note(req):
                logger.info("[请求清洗] 纯文字消息，已注入图片归属提示")
        except Exception:
