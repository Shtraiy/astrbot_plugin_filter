...
- AstrBot 新版 `context.llm_generate()` 调用使用配置页选择的服务提供商，并传入本地图片路径。
...
- 视觉识别、偷取分类、智能回复和后台索引分别通过 AstrBot `select_provider` 服务提供商选择器配置；回复提供商未填写时安全回退到情景分类提供商。
...
- `python -m unittest discover -s tests -v`：22 项通过，1 项因未安装 Pillow 跳过。
