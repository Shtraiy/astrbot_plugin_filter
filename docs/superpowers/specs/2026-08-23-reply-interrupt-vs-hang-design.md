...
| 规划期，活跃事件**无** `provider_request` | `active_event.get_extra("provider_request") is None` | 打断：stop 旧 agent + supersede 旧事件 + 合并重生成 |
| 规划期，活跃事件**有** `provider_request` | `... is not None` | 悬挂：不 stop、不 supersede、不合并；让核心 follow-up 接管，当前回复发完 |
| 规划期，任何状态 + 修正词 | `is_correction_follow_up(text)` | 一律打断 + 合并重生成 |

## 3. 关键机制依据

### 3.1 `provider_request` 标记

AstrBot 核心在构建 provider 请求后把 `provider_request` 写入事件 extra（`internal.py` 中 `has_provider_request = event.get_extra("provider_request") is not None`），`on_llm_request` 钩子期间即可读到。本地插件 `astrbot_plugin_meme_manager_master` 也已在 `on_llm_request` 中通过 `event.get_extra("provider_request")` 读取模型名，证明该标记可用且稳定。

### 3.2 follow-up 吞噬与 `agent_stop_requested`
...
def should_interrupt_running_reply(provider_call_started: bool, is_correction: bool) -> bool:
    """Provider not started -> interrupt; provider started -> hang, unless correction."""
...
3. `provider_call_started = active is not None and active.get_extra("provider_request") is not None`（异常时按 False 处理，安全侧为打断）；
4. 若 `active is None` 或 `should_interrupt_running_reply(provider_call_started, is_correction_follow_up(event.message_str))` 为 False → **提前 return，不 stop、不 promote、不合并**；
...
- `active.get_extra` 不存在/抛异常 → `provider_call_started = False`（安全侧：打断），并记录 debug 日志；
...
4. 集成 `on_message`：provider 未开始 → stop + promote；provider 已开始且非修正 → 不 stop、不 promote；
5. 集成 `on_waiting_llm_request`：悬挂分支不合并、不 supersede、planning state 被清理；
...
版本：v3.0.14，更新 `metadata.yaml` 与 README badge。
