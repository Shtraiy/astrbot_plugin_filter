...
- LLM 智能分段：超过 `segment_min_chars`（默认 150 字）的纯文本回复，调用设置面板选择的 provider（`_special: select_provider` 下拉）按语义拆分；拆分结果经"拼接后与原文零改动"校验，段间随机延迟逐条发送；未配置 provider、超时或校验失败时自动回退规则分段。
...
- 智能分段仅处理纯文本回复；含图片/文件/引用的回复保持单条发送。
