...
- LLM 智能分段：超过 `segment_min_chars`（默认 80 字）的纯文本回复，调用设置面板选择的 provider（`_special: select_provider` 下拉）按语义拆分；带引用的回复也会分段（引用保留在第一条消息上），带图片/文件的回复不分段；拆分结果经"拼接后与原文零改动"校验，段间随机延迟逐条发送；未配置 provider、超时或校验失败时自动回退规则分段。
...
| 分段最小字数 | 80 | 纯文本回复达到该字数才尝试智能分段（建议 60~200） |
